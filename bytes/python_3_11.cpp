#include "bytecode_map.h"

BEGIN_MAP(3, 11)
    MAP_OP(0, CACHE)
    MAP_OP(1, POP_TOP)
    MAP_OP(2, PUSH_NULL)
    MAP_OP(3, BINARY_OP_ADAPTIVE)
    MAP_OP(4, BINARY_OP_ADD_FLOAT)
    MAP_OP(5, BINARY_OP_ADD_INT)
    MAP_OP(6, BINARY_OP_ADD_UNICODE)
    MAP_OP(7, BINARY_OP_INPLACE_ADD_UNICODE)
    MAP_OP(8, BINARY_OP_MULTIPLY_FLOAT)
    MAP_OP(9, NOP)
    MAP_OP(10, UNARY_POSITIVE)
    MAP_OP(11, UNARY_NEGATIVE)
    MAP_OP(12, UNARY_NOT)
    MAP_OP(13, BINARY_OP_MULTIPLY_INT)
    MAP_OP(14, BINARY_OP_SUBTRACT_FLOAT)
    MAP_OP(15, UNARY_INVERT)
    MAP_OP(16, BINARY_OP_SUBTRACT_INT)
    MAP_OP(17, BINARY_SUBSCR_ADAPTIVE)
    MAP_OP(18, BINARY_SUBSCR_DICT)
    MAP_OP(19, BINARY_SUBSCR_GETITEM)
    MAP_OP(20, BINARY_SUBSCR_LIST_INT)
    MAP_OP(21, BINARY_SUBSCR_TUPLE_INT)
    MAP_OP(22, CALL_ADAPTIVE_A)
    MAP_OP(23, CALL_PY_EXACT_ARGS_A)
    MAP_OP(24, CALL_PY_WITH_DEFAULTS_A)
    MAP_OP(25, BINARY_SUBSCR)
    MAP_OP(26, COMPARE_OP_ADAPTIVE_A)
    MAP_OP(27, COMPARE_OP_FLOAT_JUMP_A)
    MAP_OP(28, COMPARE_OP_INT_JUMP_A)
    MAP_OP(29, COMPARE_OP_STR_JUMP_A)
    MAP_OP(30, GET_LEN)
    MAP_OP(31, MATCH_MAPPING)
    MAP_OP(32, MATCH_SEQUENCE)
    MAP_OP(33, MATCH_KEYS)
    MAP_OP(34, EXTENDED_ARG_QUICK_A)
    MAP_OP(35, PUSH_EXC_INFO)
    MAP_OP(36, CHECK_EXC_MATCH)
    MAP_OP(37, CHECK_EG_MATCH)
    MAP_OP(38, JUMP_BACKWARD_QUICK_A)
    MAP_OP(39, LOAD_ATTR_ADAPTIVE_A)
    MAP_OP(40, LOAD_ATTR_INSTANCE_VALUE_A)
    MAP_OP(41, LOAD_ATTR_MODULE_A)
    MAP_OP(42, LOAD_ATTR_SLOT_A)
    MAP_OP(43, LOAD_ATTR_WITH_HINT_A)
    MAP_OP(44, LOAD_CONST__LOAD_FAST_A)
    MAP_OP(45, LOAD_FAST__LOAD_CONST_A)
    MAP_OP(46, LOAD_FAST__LOAD_FAST_A)
    MAP_OP(47, LOAD_GLOBAL_ADAPTIVE_A)
    MAP_OP(48, LOAD_GLOBAL_BUILTIN_A)
    MAP_OP(49, WITH_EXCEPT_START)
    MAP_OP(50, GET_AITER)
    MAP_OP(51, GET_ANEXT)
    MAP_OP(52, BEFORE_ASYNC_WITH)
    MAP_OP(53, BEFORE_WITH)
    MAP_OP(54, END_ASYNC_FOR)
    MAP_OP(55, LOAD_GLOBAL_MODULE_A)
    MAP_OP(56, LOAD_METHOD_ADAPTIVE_A)
    MAP_OP(57, LOAD_METHOD_CLASS_A)
    MAP_OP(58, LOAD_METHOD_MODULE_A)
    MAP_OP(59, LOAD_METHOD_NO_DICT_A)
    MAP_OP(60, STORE_SUBSCR)
    MAP_OP(61, DELETE_SUBSCR)
    MAP_OP(62, LOAD_METHOD_WITH_DICT_A)
    MAP_OP(63, LOAD_METHOD_WITH_VALUES_A)
    MAP_OP(64, PRECALL_ADAPTIVE_A)
    MAP_OP(65, PRECALL_BOUND_METHOD_A)
    MAP_OP(66, PRECALL_BUILTIN_CLASS_A)
    MAP_OP(67, PRECALL_BUILTIN_FAST_WITH_KEYWORDS_A)
    MAP_OP(68, GET_ITER)
    MAP_OP(69, GET_YIELD_FROM_ITER)
    MAP_OP(70, PRINT_EXPR)
    MAP_OP(71, LOAD_BUILD_CLASS)
    MAP_OP(72, PRECALL_METHOD_DESCRIPTOR_FAST_WITH_KEYWORDS_A)
    MAP_OP(73, PRECALL_NO_KW_BUILTIN_FAST_A)
    MAP_OP(74, LOAD_ASSERTION_ERROR)
    MAP_OP(75, RETURN_GENERATOR)
    MAP_OP(76, PRECALL_NO_KW_BUILTIN_O_A)
    MAP_OP(77, PRECALL_NO_KW_ISINSTANCE_A)
    MAP_OP(78, PRECALL_NO_KW_LEN_A)
    MAP_OP(79, PRECALL_NO_KW_LIST_APPEND_A)
    MAP_OP(80, PRECALL_NO_KW_METHOD_DESCRIPTOR_FAST_A)
    MAP_OP(81, PRECALL_NO_KW_METHOD_DESCRIPTOR_NOARGS_A)
    MAP_OP(82, LIST_TO_TUPLE)
    MAP_OP(83, RETURN_VALUE)
    MAP_OP(84, IMPORT_STAR)
    MAP_OP(85, SETUP_ANNOTATIONS)
    MAP_OP(86, YIELD_VALUE)
    MAP_OP(87, ASYNC_GEN_WRAP)
    MAP_OP(88, PREP_RERAISE_STAR)
    MAP_OP(89, POP_EXCEPT)
    MAP_OP(90, STORE_NAME_A)
    MAP_OP(91, DELETE_NAME_A)
    MAP_OP(92, UNPACK_SEQUENCE_A)
    MAP_OP(93, FOR_ITER_A)
    MAP_OP(94, UNPACK_EX_A)
    MAP_OP(95, STORE_ATTR_A)
    MAP_OP(96, DELETE_ATTR_A)
    MAP_OP(97, STORE_GLOBAL_A)
    MAP_OP(98, DELETE_GLOBAL_A)
    MAP_OP(99, SWAP_A)
    MAP_OP(100, LOAD_CONST_A)
    MAP_OP(101, LOAD_NAME_A)
    MAP_OP(102, BUILD_TUPLE_A)
    MAP_OP(103, BUILD_LIST_A)
    MAP_OP(104, BUILD_SET_A)
    MAP_OP(105, BUILD_MAP_A)
    MAP_OP(106, LOAD_ATTR_A)
    MAP_OP(107, COMPARE_OP_A)
    MAP_OP(108, IMPORT_NAME_A)
    MAP_OP(109, IMPORT_FROM_A)
    MAP_OP(110, JUMP_FORWARD_A)
    MAP_OP(111, JUMP_IF_FALSE_OR_POP_A)
    MAP_OP(112, JUMP_IF_TRUE_OR_POP_A)
    MAP_OP(113, PRECALL_NO_KW_METHOD_DESCRIPTOR_O_A)
    MAP_OP(114, POP_JUMP_FORWARD_IF_FALSE_A)
    MAP_OP(115, POP_JUMP_FORWARD_IF_TRUE_A)
    MAP_OP(116, LOAD_GLOBAL_A)
    MAP_OP(117, IS_OP_A)
    MAP_OP(118, CONTAINS_OP_A)
    MAP_OP(119, RERAISE_A)
    MAP_OP(120, COPY_A)
    MAP_OP(121, PRECALL_NO_KW_STR_1_A)
    MAP_OP(122, BINARY_OP_A)
    MAP_OP(123, SEND_A)
    MAP_OP(124, LOAD_FAST_A)
    MAP_OP(125, STORE_FAST_A)
    MAP_OP(126, DELETE_FAST_A)
    MAP_OP(127, PRECALL_NO_KW_TUPLE_1_A)
    MAP_OP(128, POP_JUMP_FORWARD_IF_NOT_NONE_A)
    MAP_OP(129, POP_JUMP_FORWARD_IF_NONE_A)
    MAP_OP(130, RAISE_VARARGS_A)
    MAP_OP(131, GET_AWAITABLE_A)
    MAP_OP(132, MAKE_FUNCTION_A)
    MAP_OP(133, BUILD_SLICE_A)
    MAP_OP(134, JUMP_BACKWARD_NO_INTERRUPT_A)
    MAP_OP(135, MAKE_CELL_A)
    MAP_OP(136, LOAD_CLOSURE_A)
    MAP_OP(137, LOAD_DEREF_A)
    MAP_OP(138, STORE_DEREF_A)
    MAP_OP(139, DELETE_DEREF_A)
    MAP_OP(140, JUMP_BACKWARD_A)
    MAP_OP(141, PRECALL_NO_KW_TYPE_1_A)
    MAP_OP(142, CALL_FUNCTION_EX_A)
    MAP_OP(143, PRECALL_PYFUNC_A)
    MAP_OP(144, EXTENDED_ARG_A)
    MAP_OP(145, LIST_APPEND_A)
    MAP_OP(146, SET_ADD_A)
    MAP_OP(147, MAP_ADD_A)
    MAP_OP(148, LOAD_CLASSDEREF_A)
    MAP_OP(149, COPY_FREE_VARS_A)
    MAP_OP(150, RESUME_QUICK_A)
    MAP_OP(151, RESUME_A)
    MAP_OP(152, MATCH_CLASS_A)
    MAP_OP(153, STORE_ATTR_ADAPTIVE_A)
    MAP_OP(154, STORE_ATTR_INSTANCE_VALUE_A)
    MAP_OP(155, FORMAT_VALUE_A)
    MAP_OP(156, BUILD_CONST_KEY_MAP_A)
    MAP_OP(157, BUILD_STRING_A)
    MAP_OP(158, STORE_ATTR_SLOT_A)
    MAP_OP(159, STORE_ATTR_WITH_HINT_A)
    MAP_OP(160, LOAD_METHOD_A)
    MAP_OP(161, STORE_FAST__LOAD_FAST_A)
    MAP_OP(162, LIST_EXTEND_A)
    MAP_OP(163, SET_UPDATE_A)
    MAP_OP(164, DICT_MERGE_A)
    MAP_OP(165, DICT_UPDATE_A)
    MAP_OP(166, PRECALL_A)
    MAP_OP(167, STORE_FAST__STORE_FAST_A)
    MAP_OP(168, STORE_SUBSCR_ADAPTIVE)
    MAP_OP(169, STORE_SUBSCR_DICT)
    MAP_OP(170, STORE_SUBSCR_LIST_INT)
    MAP_OP(171, CALL_A)
    MAP_OP(172, KW_NAMES_A)
    MAP_OP(173, POP_JUMP_BACKWARD_IF_NOT_NONE_A)
    MAP_OP(174, POP_JUMP_BACKWARD_IF_NONE_A)
    MAP_OP(175, POP_JUMP_BACKWARD_IF_FALSE_A)
    MAP_OP(176, POP_JUMP_BACKWARD_IF_TRUE_A)
    MAP_OP(177, UNPACK_SEQUENCE_ADAPTIVE_A)
    MAP_OP(178, UNPACK_SEQUENCE_LIST_A)
    MAP_OP(179, UNPACK_SEQUENCE_TUPLE_A)
    MAP_OP(180, UNPACK_SEQUENCE_TWO_TUPLE_A)
END_MAP()
